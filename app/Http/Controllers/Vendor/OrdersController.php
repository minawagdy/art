...
//        }
    ...
    public function getIndex(Request $request, $id=0)
    {
        ...
        // dd($request->all());
        // authorize('view-' . $this->module);
            $rows = Order::where('provider_id',$this->vendor->user()->id)->with("provider","provider.images","status","driver","driver.zoneObj");
            ...
            // dd($status);
        return view('vendor.orders.index',compact('rows','status','all_gov','all_zones'));
        // return response()->json(['isSuccessed' => true, "data" => $rows, 'error' => null], 200);
    }
    ...
    public function getView($id) {
        ...
        $row = Order::with("provider","provider.images","customer","payment","status","address","orderProducts","orderProducts.product","orderProducts.price","driver","driver.zoneObj")->where([['provider_id',$this->vendor->user()->id],['id',$id]])->first();
        if ($row) {
            return view('vendor.orders.details',compact('row'));
        }
        else
        {
