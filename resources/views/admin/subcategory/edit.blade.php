...
<div class="intro-y col-span-12 lg:col-span-6 mt-5">
    ...
    <div class="intro-y box">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60 dark:border-darkmode-400">
            ...
            <form name="add-blog-post-form" id="add-blog-post-form" method="post" action="{{ route('subcategory.update', ['id' => $subcategory->id]) }}" enctype="multipart/form-data">
        ...
        <div id="vertical-form" class="p-5">
            <div class="preview">
                <div>
                    ...
                    <input id="vertical-form-1" type="text" class="form-control" name="title_en" value="{{($subcategory->title_en)}}">
                </div>
                @error('title_en')
                ...
                <div class="mt-3">
                    ...
                    <input id="vertical-form-2" type="text" class="form-control"  name="title_ar" value="{{($subcategory->title_ar)}}">
                </div>
                @error('title_ar')
                <div class="error">{{ $message }}</div>
                @enderror
                ...
                <div class="mt-3">
                    <label for="vertical-form-2" class="form-label">Category</label>
                    <select id="vertical-form-2" type="text" class="form-control"  name="category_id">
                        @foreach ($categories as $category)
                        @if($subcategory->category->id == $category->id)
                        <option value="{{$category->id}}" selected>{{$category->title_en}}</option>
                            @else
